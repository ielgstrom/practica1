...
def generate_result_table(country_links, list_headers_table=[0, 1, 2], list_header_table=[0, 1, 2], link_definition=lambda x:x):
...
def get_debt_yearly_data():
    ...
    df_result['Deuda Per Cápita'] = pd.to_numeric(df_result['Deuda Per Cápita'].str.replace('.', '', regex=False)
                                              .str.replace('€', '', regex=False)
                                                  .str.replace('\u00A0', '', regex=False))
    df_result.rename(columns={'Deuda total (M.€)': 'Deute Total', 'Deuda (%PIB)': 'Deute PIB','Deuda Per Cápita':'Deute Per Capita'}, inplace=True)
    return df_result
